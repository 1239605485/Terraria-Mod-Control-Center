...
std::atomic<unsigned long long> g_plain_swap_count{0};
...
void flush_native_probe_markers_on_game_thread() {
    ...
    static const Marker markers[] = {
        ...
        {PROBE_DAMAGE_KHR_SEEN, "DAMAGE_KHR_SEEN"},
        {PROBE_DAMAGE_EXT_SEEN, "DAMAGE_EXT_SEEN"},
...
void main_update_postfix(
    ...
    if (!g_diag_dimensions_emitted.load(std::memory_order_acquire)) {
        ...
        if (sw > 0 && sh > 0 && vw > 0 && vh > 0) {
            ...
            emit_tef_probe_marker(marker);
            std::snprintf(marker, sizeof(marker), "PROBE_PIXEL_0x%08X", g_probe_pixel_rgba.load(std::memory_order_relaxed));
            emit_tef_probe_marker(marker);
...
void draw_fb0_probe_marker(int surface_w, int surface_h) {
    ...
    // Read back the centre of the magenta diagnostic square. This separates
    // "GL accepted the commands" from "the target buffer really changed".
    unsigned char pixel[4] = {0, 0, 0, 0};
    glReadPixels(22, 22, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, pixel);
    const unsigned int packed =
        (static_cast<unsigned int>(pixel[0]) << 24) |
        (static_cast<unsigned int>(pixel[1]) << 16) |
        (static_cast<unsigned int>(pixel[2]) << 8) |
        static_cast<unsigned int>(pixel[3]);
    g_probe_pixel_rgba.store(packed, std::memory_order_relaxed);
    if (pixel[0] > 200 && pixel[1] < 80 && pixel[2] > 200)
        g_native_probe_flags.fetch_or(PROBE_PIXEL_READBACK_OK, std::memory_order_release);
    else
        g_native_probe_flags.fetch_or(PROBE_PIXEL_READBACK_FAIL, std::memory_order_release);
...
void draw_overlay(int surface_w, int surface_h) {
    ...
    // Make all overlay commands visible to EGL before the swap/damage call.
    glFlush();
...
EGLBoolean hooked_egl_swap_buffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    g_plain_swap_count.fetch_add(1, std::memory_order_relaxed);
...
EGLBoolean hooked_egl_swap_buffers_with_damage_khr(EGLDisplay dpy, EGLSurface surface, EGLint* rects, EGLint n_rects) {
    ...
    g_inside_swap_proxy = true;
    g_khr_damage_swap_count.fetch_add(1, std::memory_order_relaxed);
    g_forced_full_damage_count.fetch_add(1, std::memory_order_relaxed);
    g_native_probe_flags.fetch_or(
        PROBE_DAMAGE_KHR_SEEN | PROBE_FULL_DAMAGE_FORCED,
        std::memory_order_release
    );
    render_before_native_swap(dpy, surface, "eglSwapBuffersWithDamageKHR");
    // n_rects == 0 means the complete surface is damaged. Passing Terraria's
    // original rectangles can exclude the newly drawn overlay indefinitely.
    EGLBoolean result = g_original_egl_swap_buffers_with_damage_khr
        ? g_original_egl_swap_buffers_with_damage_khr(dpy, surface, nullptr, 0)
        : EGL_FALSE;
    g_inside_swap_proxy = false;
    return result;
}
...
EGLBoolean hooked_egl_swap_buffers_with_damage_ext(EGLDisplay dpy, EGLSurface surface, EGLint* rects, EGLint n_rects) {
    ...
    g_inside_swap_proxy = true;
    g_ext_damage_swap_count.fetch_add(1, std::memory_order_relaxed);
    g_forced_full_damage_count.fetch_add(1, std::memory_order_relaxed);
    g_native_probe_flags.fetch_or(
        PROBE_DAMAGE_EXT_SEEN | PROBE_FULL_DAMAGE_FORCED,
        std::memory_order_release
    );
    ...
    EGLBoolean result = g_original_egl_swap_buffers_with_damage_ext
        ? g_original_egl_swap_buffers_with_damage_ext(dpy, surface, nullptr, 0)
        : EGL_FALSE;
    g_inside_swap_proxy = false;
    return result;
}
...
extern "C" void mod_control_center_cleanup(void) {
    ...
    append_runtime_logf(
        ...
        g_khr_damage_swap_count.load(std::memory_order_relaxed),
        g_ext_damage_swap_count.load(std::memory_order_relaxed),
        g_forced_full_damage_count.load(std::memory_order_relaxed),
        g_probe_pixel_rgba.load(std::memory_order_relaxed)
    );
}
